Partial Class Tests
    Public Shared Function Test_CLEAR_CAL() As Boolean
        ...
        For Each UUT In UUTs
            ...
            Try
                ...
                If UUT("SN").Text = "" Or UUT("FAILED") Then Continue For
                Form1.AppendText("Clearing calibration", UUT:=UUT)
            ...
            Catch ex As Exception
                Form1.AppendText(UUT("SN").Text + ": Test_CLEAR_CAL() caught" + ex.ToString, UUT:=UUT)
                UUT("FAILED") = True
                UUT("TAB").ImageIndex = StatusColor.FAILED
                Success = False
                Continue For
    ...
    Public Shared Function Test_CLEAR_CAL0() As Boolean
        Dim Success As Boolean
        Dim results As ReturnResults
        Dim SF As New SerialFunctions
        Dim h2scan As New H2SCAN_debug
        ...
        For Each UUT In UUTs
            Try
                ...
                End If
            Catch ex As Exception
                Form1.AppendText("Test_Clear_Cal() caught" + ex.ToString, UUT:=UUT)
                UUT("FAILED") = True
                UUT("TAB").ImageIndex = StatusColor.FAILED
                Success = False
            End Try
